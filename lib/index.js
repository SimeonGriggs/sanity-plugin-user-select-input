...
import { Card, Flex, Box, Avatar, Text, Autocomplete } from '@sanity/ui';
...
forwardRef(function Icon2(props, ref) {
  ...
  return createElement(iconComponent, _objectSpread(_objectSpread({}, restProps), {}, {
    ref
  }));
});
...
function UserSelectAutocomplete(props) {
  ...
  const handleChange = React.useCallback(userId => onChange(userId ? set(userId) : unset()), [onChange]);
  const handleFilterOption = React.useCallback((query, option) => {
