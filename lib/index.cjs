...
React.forwardRef(function Icon2(props, ref) {
  const {
      symbol
    } = props,
  ...
  return React.createElement(iconComponent, _objectSpread(_objectSpread({}, restProps), {}, {
    ref
  }));
});
...
function UserSelectAutocomplete(props) {
  var _a, _b, _c;
  const {
    value,
    id,
  ...
  const projectUsers = sanityPluginUtils.useProjectUsers({
    apiVersion: "2023-01-01"
  });
  const handleChange = React__default.default.useCallback(userId => onChange(userId ? sanity.set(userId) : sanity.unset()), [onChange]);
  const handleFilterOption = React__default.default.useCallback((query, option) => {
    return option.payload.displayName.toLowerCase().indexOf(query.toLowerCase()) > -1;
  }, []);
  const userList = (projectUsers == null ? void 0 : projectUsers.length) ? projectUsers.filter(user => !user.displayName.includes("(Robot)")) : [];
  const userSelected = userList.find(user => user.id === value);
  const userRender = (_a = userSelected == null ? void 0 : userSelected.displayName) != null ? _a : "No user found with this ID: ".concat(value);
  return /* @__PURE__ */jsxRuntime.jsx(ui.Autocomplete, {
    icon: UsersIcon,
    radius: 1,
    id: "user-select-".concat(id),
    ...
    readOnly,
    renderOption: Option,
    renderValue: () => (userList == null ? void 0 : userList.length) > 0 && value && userRender ? userRender : "",
    padding: 3,
    fontSize: 2,
    placeholder: (_c = (_b = props.schemaType.type) == null ? void 0 : _b.placeholder) != null ? _c : "Search Users",
    value: value != null ? value : ""
  });
}
const userSelect$1 = sanity.defineType({
  name: "userSelect",
  type: "string",
  components: {
...
const userSelect = sanity.definePlugin(() => {
  return {
